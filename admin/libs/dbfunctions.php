...
class dbobject extends validation
{
	...
	public function connection(){
		$this->db_connect = mysqli_connect($this->db_server, $this->db_user, $this->db_password) or trigger_error(mysqli_error($this->db_connect));
    ...
    function EncryptData($username,$userpassword)
    {
	 ...
	 {
		$this->db_connect= $this->connection();
		...
		//  var_dump(mysqli_fetch_assoc($result));
		 $count  = mysqli_affected_rows($this->db_connect);
	...
	public function queryMysql($sql) 
	{
 		if ($this->debug === false)
		{
 			try {
 				...
 				if ($result === false)
				{
 					throw new Exception('MySQL Query Error: ' . mysqli_error($this->db_connect));
					//$result = '-1';
 				}
 				return $result;
 			}
	...
	function SaveTransEdit($tbl,$inpFds,$inpFdsVals,$operation)
	{
		...
		if(mysqli_query($this->db_connect, $query_data)or die(mysqli_error($this->db_connect)))
		{
			$resp += 1;
...
*/		$daty = @date('Y-m-d H:i:s');
		...
		$ip = $_SERVER['REMOTE_ADDR'];
		//$query2 = "insert into audit_trail  values('','$tbl','$trail_appl','$afftd','$intVals','$updatedVals','Edit','$daty','$officer','$ip')";
		if(mysqli_query($this->db_connect, $query_data))//or die(mysql_error()))
		{
			$resp += 1;
			//if(mysql_query($query2))	$resp += 1;
			//else	$resp = -2;
...
///////////////////////////////////////////////////////		
	function exister($table,$field1,$field2,$value1,$value2)		
	{
		...
		//////////////////////////
		$existed = mysqli_query("SELECT * FROM $table WHERE $field1='$value1' and $field2='$value2'")or die('Inavlid Exist Query'. mysqli_error($this->db_connect));
		$no = counter($existed) ;
		return $no;
	}
...
function getcheckdetails($user,$password) {
	...
	$numrows = mysqli_affected_rows($this->db_connect);
...
function getuserip_status($user){
	...
	//echo $qr1;
			$mq1 = mysqli_query($this->db_connect, $qr1);
			$mn1 = mysqli_num_rows($mq1);
			$label = 0;
			if($mn1>0){
				$label = $mn1;
...
function getlastlogin($user){
	...
	//check to see if the user has logged in to this system or another system
			$qr = " SELECT AUDIT_IP, AUDIT_T_IN FROM audit_trail_account WHERE AUDIT_USER='".$user."' AND 
					...
					//echo $qr;
			$mq = mysqli_query($this->db_connect, $qr);
			$mn = mysqli_num_rows($mq);
			if($mn>0){
				$rr = mysqli_fetch_array($mq);
				$the_ip = $rr["AUDIT_IP"];
...
//		echo $query;
	...
	//echo $query;
	// $result = mysqli_query($this->db_connect, $query);
	// $numrows = mysqli_num_rows($this->db_connect);
	$result = $this->db_query($query);
	if(count($result) > 0){
		$label = $result[0][$ret_val];
	}
	return $label;
	}
...
//                    		file_put_contents("june.txt",$query_data);
			...
			function doTeacher($operation,$state,$lga,$username,$firstname,$lastname,$teach_address,$teach_mail,$teach_phone,$role_id,$sch_id,$sch_name,$teach_id,$status,$flag,$teach_role,$posted_user,$gender)
	{
			...
			function doStudent($operation,$state,$lga,$username,$firstname,$lastname,$address,$email,$phone,$role_id,$sch_id,$sch_name,$std_id,$status,$student_role,$posted_user,$gender,$class_cat_id,$class_id,$parent_id,$middle_name)
	{
...
$returnCount=-9;	
	...
	function save_result($appID,$reg_no,$std_name,$sex,$age,$ca1,$ca2,$exam,$total,$subject,$no_class,$class,$seesion,$term,$teacher,$status,$posted_user,$sch_id,$created,$client_ip){
	 $operation =$_REQUEST['operation']=="edit"?$_REQUEST['operation']:'new';
$returnCount=-9;	
	if ($operation=='new')
	{
